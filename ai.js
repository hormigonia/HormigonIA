...
function getCalculatorStateContext() {
    ...
    const densStone = parseFloat(document.getElementById("densStone").value);
    const coefStone = parseFloat(document.getElementById("coefStone").value);
    const moistStone = parseFloat(document.getElementById("moistStone").value);
    ...
    const globalStonePass = window.lastCalculatedPassingStone || [];
    ...
    for (let i = 0; i < allSieveLabels.length; i++) {
        ...
        stoneSieves.push({
            sieve: allSieveLabels[i],
            retainedGrams: parseFloat(stoneInputs[i].value) || 0,
            passingPercent: globalStonePass[i] !== undefined ? globalStonePass[i] : 100
        });
    }
    ...
    const resStone = document.getElementById("resStone").innerText;
    ...
    return {
        ...
        materialsLabProperties: {
            ...
            sand: { bulkDensity: densSand, yieldCoef: coefSand, moistureLperM3: moistSand },
            stone: { bulkDensity: densStone, yieldCoef: coefStone, moistureLperM3: moistStone }
        },
        ...
        calculatedRecipeForBatchVolume: {
            ...
            wetStoneKg: parseFloat(resStone)
        },
...
function applyUIUpdates(updates) {
    ...
    const simpleInputs = [
        ...
        "densSand", "moistSand", "densStone", "moistStone",
        "densCement", "coefCement", "coefSand", "coefStone",
